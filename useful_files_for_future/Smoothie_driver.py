...
from opentrons.drivers.smoothie_drivers import HOMED_POSITION, Y_BOUND_OVERRIDE
...
class LIMIT_POSITIONS(float,Enum):
    # X axis motors
    X_MAX: 418.0
    X_MIN: 20
    # Y axis motors
...
class SmoothieDriver_3_0_0:
    def __init__(
        ...
        self._current_settings: Dict[str, Dict[str, float]] = {
            'now': cast(
                Dict[str, float],
                current_for_revision(config.low_current, self._gpio_chardev.board_rev).copy()),
            'saved': cast(
                Dict[str, float],
...
if __name__ == '__main__':
    obj = SmoothieDriver_3_0_0(config=build_config({}))
    obj.connect('/dev/cu.usbserial-AQ00KSUJ')
    #obj.home()
    print(obj._position)
