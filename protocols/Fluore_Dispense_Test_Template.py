...
'''
This protocol is used to add 20 uL (20000 nL) of borate buffer to wells prior to addition of fluorescien.
It is also set up to test two different methods of delivery (pick up and drop off one at a time, or pick up in batches).
It is designed such that the 20 uL buffer can be added to wells via 12-channel pipette (target wells). 
'''
...
for well in group_01_wells:
    myProtocol.dispense_to(0, dip_water) # wash off outside of tip
